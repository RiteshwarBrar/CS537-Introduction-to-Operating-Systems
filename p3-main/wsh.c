			...
			//ppid=getpid();
				...
				if((pid = fork())==0){
					...
					//printf("BB%d \n",numc);
						...
						tcsetpgrp(STDIN_FILENO, fpid);
									
								if(waitpid(fpid, &stat, bg==1?WNOHANG:WUNTRACED)>0) //
									if (WIFEXITED(stat) || WIFSIGNALED(stat))
						    				rem_jobs(&jobs[jid-1]);
